...
import { filterName, KotlinWriter, toException, toPackageCase } from "./common.ts"
...
export function writeOperation(
    ...
    switch (operation.response?.type) {
        case "application/json": {
            ...
            crossRef.add(`io.portone.sdk.server.${toPackageCase(category)
                }.${operation.response.schema}`)
            crossRef.add(`io.portone.sdk.server.${operation.response.schema}`)
            writer.writeLine(
                `): ${operation.response.schema} {`,
            )
            futureWriter.writeLine(`): CompletableFuture<${operation.response.schema}> = GlobalScope.future { ${operation.id}(${paramList}) }`)
            break
        }
        case "text/csv":
            crossRef.add("kotlin.String")
            writer.writeLine("): String {")
            futureWriter.writeLine(`): String = GlobalScope.future { ${operation.id}(${paramList}) }`)
            break
        case undefined:
            writer.writeLine(") {")
            futureWriter.writeLine(`) = GlobalScope.future { ${operation.id}(${paramList}) }`)
            break
        default:
    ...
    if (operation.params.body) {
        if (operation.params.body.type !== 'ref')
            throw new Error("unsupported request body type", { cause: { definition: operation.params.body } })
    ...
    writer.indent()
    const path = operation.path.split("/").slice(1).map((segment) => segment.startsWith("{") && segment.endsWith("}") ? segment.slice(1, -1) : `"${segment}"`).join(", ")
    writer.writeLine(`appendPathSegments(${path})`)
    for (const property of operation.params.query) {
        if (property.name === "requestBody") {
            writer.writeLine(`parameters.append("requestBody", json.encodeToString(requestBody))`)
        } else if (property.required) {
            writer.writeLine(`parameters.append("${property.name}", ${property.name})`)
        } else {
            writer.writeLine(`if (${property.name} != null) parameters.append("${property.name}", ${property.name})`)
        }
    }
    writer.outdent()
    writer.writeLine("}")
    ...
    for (const error of errors) {
        writer.writeLine(`is ${error.name} -> throw ${toException(error.name)}(httpBodyDecoded)`)
    }
    writer.outdent()
    writer.writeLine("}")
    writer.outdent()
    ...
    switch (operation.response?.type) {
        case "application/json":
            ...
            writer.indent()
            writer.writeLine(`json.decodeFromString<${operation.response.schema}>(httpBody)`)
            writer.outdent()
            writer.writeLine("}")
            writer.writeLine("catch (_: Exception) {")
            writer.indent()
...
function writePropertyList(
    ...
    for (const property of properties) {
        ...
        const wrapOptional = (type: string) => property.required ? type : `${type}? = null`
