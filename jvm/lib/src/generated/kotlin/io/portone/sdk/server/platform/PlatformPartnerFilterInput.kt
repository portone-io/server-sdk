...
public data class PlatformPartnerFilterInput(
   ...
   */
  val types: List<PlatformPartnerTypeName>? = null,
  /**
   * 플랫폼 파트너 과세 유형
   *
   * 하나 이상의 값이 존재하는 경우 해당 리스트에 포함되는 과세 유형을 가진 파트너만 조회합니다.
   */
  val taxationTypes: List<PlatformPartnerTaxationType>? = null,
  /**
   * 플랫폼 파트너 연동 사업자 연결 상태
   *
   * 하나 이상의 값이 존재하는 경우 해당 리스트에 포함되는 연동 사업자 연결 상태를 가진 파트너만 조회합니다.
   */
  val memberCompanyConnectionStatuses: List<PlatformPartnerMemberCompanyConnectionStatus>? = null,
  /** 검색 키워드 */
