...
class ScheduledPaymentSchedule:
    """결제 예약 완료 상태
    """
    status: Literal["SCHEDULED"] = field(repr=False)
    """결제 예약 건 상태
    """
    id: str
    """결제 예약 건 아이디
...
def _serialize_scheduled_payment_schedule(obj: ScheduledPaymentSchedule) -> Any:
    entity = {}
...
def _deserialize_scheduled_payment_schedule(obj: Any) -> ScheduledPaymentSchedule:
    ...
    status = obj["status"]
    if status != "SCHEDULED":
        raise ValueError(f"{repr(status)} is not 'SCHEDULED'")
    if "id" not in obj:
    ...
    if "products" in obj:
        ...
        products = None
    return ScheduledPaymentSchedule(status, id, merchant_id, store_id, payment_id, billing_key, order_name, is_cultural_expense, is_escrow, customer, custom_data, total_amount, currency, created_at, time_to_pay, tax_free_amount, vat_amount, installment_month, notice_urls, products)
