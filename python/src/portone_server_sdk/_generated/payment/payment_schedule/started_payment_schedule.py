...
class StartedPaymentSchedule:
    """결제 시작 상태
    """
    status: Literal["STARTED"] = field(repr=False)
    """결제 예약 건 상태
    """
    id: str
    """결제 예약 건 아이디
...
def _deserialize_started_payment_schedule(obj: Any) -> StartedPaymentSchedule:
    ...
    status = obj["status"]
    if status != "STARTED":
        raise ValueError(f"{repr(status)} is not 'STARTED'")
    if "id" not in obj:
    ...
    if "products" in obj:
        ...
        products = None
    return StartedPaymentSchedule(status, id, merchant_id, store_id, payment_id, billing_key, order_name, is_cultural_expense, is_escrow, customer, custom_data, total_amount, currency, created_at, time_to_pay, started_at, tax_free_amount, vat_amount, installment_month, notice_urls, products)
