...
def _serialize_cancelled_payment_event(obj: CancelledPaymentEvent) -> Any:
...
def _deserialize_cancelled_payment_event(obj: Any) -> CancelledPaymentEvent:
    ...
    type = obj["type"]
    if type != "CANCELLED":
        raise ValueError(f"{repr(type)} is not 'CANCELLED'")
    if "id" not in obj:
    ...
    if "promotionId" in obj:
        ...
        promotion_id = None
    return CancelledPaymentEvent(id, payment_id, transaction_id, merchant_id, store_id, channel, version, requested_at, created_at, order_name, total_amount, currency, customer, event_amount, origin, method, channel_group, schedule_id, webhooks, is_cultural_expense, escrow, products, product_count, custom_data, country, pg_tx_id, cash_receipt, cash_receipt_issuance_status, receipt_url, cancellation, promotion_id)
