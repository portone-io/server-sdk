...
def _serialize_paid_payment_event(obj: PaidPaymentEvent) -> Any:
...
def _deserialize_paid_payment_event(obj: Any) -> PaidPaymentEvent:
    ...
    type = obj["type"]
    if type != "PAID":
        raise ValueError(f"{repr(type)} is not 'PAID'")
    if "id" not in obj:
    ...
    if "promotionId" in obj:
        ...
        promotion_id = None
    return PaidPaymentEvent(id, payment_id, transaction_id, merchant_id, store_id, channel, version, requested_at, created_at, order_name, total_amount, currency, customer, disputes, event_amount, origin, method, channel_group, schedule_id, webhooks, is_cultural_expense, escrow, products, product_count, custom_data, country, pg_tx_id, pg_response, cash_receipt, cash_receipt_issuance_status, receipt_url, promotion_id)
