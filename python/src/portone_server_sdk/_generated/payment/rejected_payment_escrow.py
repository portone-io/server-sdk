...
class RejectedPaymentEscrow:
    """구매 거절
    """
    status: Literal["REJECTED"] = field(repr=False)
    """에스크로 상태
    """
    company: str
    """택배사
...
def _serialize_rejected_payment_escrow(obj: RejectedPaymentEscrow) -> Any:
    entity = {}
...
def _deserialize_rejected_payment_escrow(obj: Any) -> RejectedPaymentEscrow:
    ...
    status = obj["status"]
    if status != "REJECTED":
        raise ValueError(f"{repr(status)} is not 'REJECTED'")
    ...
    if "appliedAt" in obj:
        ...
        applied_at = None
    return RejectedPaymentEscrow(status, company, invoice_number, sent_at, applied_at)
