...
class DeliveredPaymentEscrow:
    """배송 완료
    """
    status: Literal["DELIVERED"] = field(repr=False)
    """에스크로 상태
    """
    company: str
    """택배사
...
def _serialize_delivered_payment_escrow(obj: DeliveredPaymentEscrow) -> Any:
    entity = {}
...
def _deserialize_delivered_payment_escrow(obj: Any) -> DeliveredPaymentEscrow:
    ...
    status = obj["status"]
    if status != "DELIVERED":
        raise ValueError(f"{repr(status)} is not 'DELIVERED'")
    ...
    if "appliedAt" in obj:
        ...
        applied_at = None
    return DeliveredPaymentEscrow(status, company, invoice_number, sent_at, applied_at)
