...
@dataclass
class PartialCancelledPaymentEvent:
    """결제 부분 취소 상태 이벤트
    """
    """결제 이벤트 종류
    """
    ...
    """거래 영수증 URL
    """
    cancellation: Optional[PaymentCancellation] = field(default=None)
    """결제 취소 정보
    """
...
def _serialize_partial_cancelled_payment_event(obj: PartialCancelledPaymentEvent) -> Any:
    ...
    entity["totalAmount"] = _serialize_payment_amount(obj.total_amount)
    ...
    if obj.cancellation is not None:
...
def _deserialize_partial_cancelled_payment_event(obj: Any) -> PartialCancelledPaymentEvent:
    if not isinstance(obj, dict):
        raise ValueError(f"{repr(obj)} is not dict")
    if "type" not in obj:
        raise KeyError(f"'type' is not in {obj}")
    type = obj["type"]
    if type != "PARTIAL_CANCELLED":
        raise ValueError(f"{repr(type)} is not 'PARTIAL_CANCELLED'")
    if "id" not in obj:
    ...
    total_amount = _deserialize_payment_amount(total_amount)
    ...
    if "eventAmount" not in obj:
        raise KeyError(f"'eventAmount' is not in {obj}")
    ...
    if "promotionId" in obj:
        ...
        promotion_id = None
    return PartialCancelledPaymentEvent(id, payment_id, transaction_id, merchant_id, store_id, channel, version, requested_at, created_at, order_name, total_amount, currency, customer, event_amount, origin, method, channel_group, schedule_id, webhooks, is_cultural_expense, escrow, products, product_count, custom_data, country, pg_tx_id, cash_receipt, cash_receipt_issuance_status, receipt_url, cancellation, promotion_id)
