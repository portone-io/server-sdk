...
def _deserialize_virtual_account_issued_payment(obj: Any) -> VirtualAccountIssuedPayment:
    ...
    if status != "VIRTUAL_ACCOUNT_ISSUED":
        raise ValueError(f"{repr(status)} is not 'VIRTUAL_ACCOUNT_ISSUED'")
    if "id" not in obj:
    ...
    if "pgTxId" in obj:
        ...
        pg_tx_id = None
    return VirtualAccountIssuedPayment(id, transaction_id, merchant_id, store_id, channel, version, requested_at, updated_at, status_changed_at, order_name, amount, currency, customer, origin, method, channel_group, schedule_id, billing_key, webhooks, promotion_id, is_cultural_expense, escrow, products, product_count, custom_data, country, pg_tx_id)
