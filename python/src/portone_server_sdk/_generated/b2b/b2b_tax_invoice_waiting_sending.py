...
@dataclass
class B2bTaxInvoiceWaitingSending:
    status: Literal["WAITING_SENDING"] = field(repr=False)
    """세금계산서 상태
    """
    tax_type: B2bTaxType
    """과세 유형
...
def _serialize_b2b_tax_invoice_waiting_sending(obj: B2bTaxInvoiceWaitingSending) -> Any:
    entity = {}
    entity["status"] = "WAITING_SENDING"
...
def _deserialize_b2b_tax_invoice_waiting_sending(obj: Any) -> B2bTaxInvoiceWaitingSending:
    if not isinstance(obj, dict):
        raise ValueError(f"{repr(obj)} is not dict")
    if "status" not in obj:
        raise KeyError(f"'status' is not in {obj}")
    status = obj["status"]
    if status != "WAITING_SENDING":
        raise ValueError(f"{repr(status)} is not 'WAITING_SENDING'")
    ...
    if "modification" in obj:
        ...
        modification = None
    return B2bTaxInvoiceWaitingSending(status, tax_type, write_date, purpose_type, supply_cost_total_amount, tax_total_amount, total_amount, remarks, supplier, recipient, send_sms, items, contacts, status_updated_at, issued_at, nts_approve_number, serial_num, book_volume, book_issue, cash_amount, check_amount, credit_amount, note_amount, supplier_document_key, recipient_document_key, modification)
