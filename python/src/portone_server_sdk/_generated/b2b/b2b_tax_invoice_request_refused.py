...
@dataclass
class B2bTaxInvoiceRequestRefused:
    status: Literal["ISSUANCE_REFUSED"] = field(repr=False)
    """세금계산서 상태
    """
    tax_type: B2bTaxType
    """과세 유형
...
def _serialize_b2b_tax_invoice_request_refused(obj: B2bTaxInvoiceRequestRefused) -> Any:
    entity = {}
    entity["status"] = "ISSUANCE_REFUSED"
...
def _deserialize_b2b_tax_invoice_request_refused(obj: Any) -> B2bTaxInvoiceRequestRefused:
    if not isinstance(obj, dict):
        raise ValueError(f"{repr(obj)} is not dict")
    if "status" not in obj:
        raise KeyError(f"'status' is not in {obj}")
    status = obj["status"]
    if status != "ISSUANCE_REFUSED":
        raise ValueError(f"{repr(status)} is not 'ISSUANCE_REFUSED'")
