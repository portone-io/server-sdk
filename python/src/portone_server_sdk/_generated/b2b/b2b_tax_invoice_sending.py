...
@dataclass
class B2bTaxInvoiceSending:
    status: Literal["SENDING"] = field(repr=False)
    """세금계산서 상태
    """
    tax_type: B2bTaxType
    """과세 유형
...
def _deserialize_b2b_tax_invoice_sending(obj: Any) -> B2bTaxInvoiceSending:
    ...
    status = obj["status"]
    if status != "SENDING":
        raise ValueError(f"{repr(status)} is not 'SENDING'")
    ...
    if "modification" in obj:
        ...
        modification = None
    return B2bTaxInvoiceSending(status, tax_type, write_date, purpose_type, supply_cost_total_amount, tax_total_amount, total_amount, remarks, supplier, recipient, send_sms, items, contacts, status_updated_at, issued_at, nts_approve_number, nts_sent_at, serial_num, book_volume, book_issue, cash_amount, check_amount, credit_amount, note_amount, supplier_document_key, recipient_document_key, modification)
