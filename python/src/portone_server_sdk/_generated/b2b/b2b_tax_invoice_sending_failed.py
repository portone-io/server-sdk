...
@dataclass
class B2bTaxInvoiceSendingFailed:
    status: Literal["SENDING_FAILED"] = field(repr=False)
    """세금계산서 상태
    """
    tax_type: B2bTaxType
    """과세 유형
...
def _serialize_b2b_tax_invoice_sending_failed(obj: B2bTaxInvoiceSendingFailed) -> Any:
    entity = {}
    entity["status"] = "SENDING_FAILED"
...
def _deserialize_b2b_tax_invoice_sending_failed(obj: Any) -> B2bTaxInvoiceSendingFailed:
    ...
    if status != "SENDING_FAILED":
        raise ValueError(f"{repr(status)} is not 'SENDING_FAILED'")
