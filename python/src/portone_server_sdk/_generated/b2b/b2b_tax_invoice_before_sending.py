...
@dataclass
class B2bTaxInvoiceBeforeSending:
    status: Literal["BEFORE_SENDING"] = field(repr=False)
    """세금계산서 상태
    """
    tax_type: B2bTaxType
    """과세 유형
...
def _serialize_b2b_tax_invoice_before_sending(obj: B2bTaxInvoiceBeforeSending) -> Any:
    entity = {}
    entity["status"] = "BEFORE_SENDING"
...
def _deserialize_b2b_tax_invoice_before_sending(obj: Any) -> B2bTaxInvoiceBeforeSending:
    if not isinstance(obj, dict):
        raise ValueError(f"{repr(obj)} is not dict")
    if "status" not in obj:
        raise KeyError(f"'status' is not in {obj}")
    status = obj["status"]
    if status != "BEFORE_SENDING":
        raise ValueError(f"{repr(status)} is not 'BEFORE_SENDING'")
