...
@dataclass
class B2bTaxInvoiceIssuanceCancelled:
    status: Literal["ISSUANCE_CANCELLED"] = field(repr=False)
    """세금계산서 상태
    """
    tax_type: B2bTaxType
    """과세 유형
...
def _serialize_b2b_tax_invoice_issuance_cancelled(obj: B2bTaxInvoiceIssuanceCancelled) -> Any:
    entity = {}
    entity["status"] = "ISSUANCE_CANCELLED"
...
def _deserialize_b2b_tax_invoice_issuance_cancelled(obj: Any) -> B2bTaxInvoiceIssuanceCancelled:
    if not isinstance(obj, dict):
        raise ValueError(f"{repr(obj)} is not dict")
    if "status" not in obj:
        raise KeyError(f"'status' is not in {obj}")
    status = obj["status"]
    if status != "ISSUANCE_CANCELLED":
        raise ValueError(f"{repr(status)} is not 'ISSUANCE_CANCELLED'")
    ...
    return B2bTaxInvoiceIssuanceCancelled(status, tax_type, write_date, purpose_type, supply_cost_total_amount, tax_total_amount, total_amount, remarks, supplier, recipient, send_sms, items, contacts, status_updated_at, issued_at, nts_approve_number, serial_num, book_volume, book_issue, cash_amount, check_amount, credit_amount, note_amount, supplier_document_key, recipient_document_key, modification, recipient_business_status, recipient_closed_suspended_date)
