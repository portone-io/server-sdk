...
@dataclass
class GetAnalyticsCardCompanyChartBody:
...
def _serialize_get_analytics_card_company_chart_body(obj: GetAnalyticsCardCompanyChartBody) -> Any:
    entity = {}
    ...
    entity["cardCompanies"] = list(map(_serialize_card_company, obj.card_companies))
    entity["excludesFromRemainders"] = list(map(_serialize_card_company, obj.excludes_from_remainders))
    return entity


def _deserialize_get_analytics_card_company_chart_body(obj: Any) -> GetAnalyticsCardCompanyChartBody:
    ...
    if "cardCompanies" not in obj:
        raise KeyError(f"'cardCompanies' is not in {obj}")
    card_companies = obj["cardCompanies"]
    if not isinstance(card_companies, list):
        raise ValueError(f"{repr(card_companies)} is not list")
    for i, item in enumerate(card_companies):
        item = _deserialize_card_company(item)
        card_companies[i] = item
    ...
    return GetAnalyticsCardCompanyChartBody(from_, until, currency, exclude_cancelled, time_granularity, card_companies, excludes_from_remainders)
