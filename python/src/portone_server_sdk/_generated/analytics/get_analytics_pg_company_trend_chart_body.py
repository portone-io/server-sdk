...
@dataclass
class GetAnalyticsPgCompanyTrendChartBody:
...
def _serialize_get_analytics_pg_company_trend_chart_body(obj: GetAnalyticsPgCompanyTrendChartBody) -> Any:
    entity = {}
...
def _deserialize_get_analytics_pg_company_trend_chart_body(obj: Any) -> GetAnalyticsPgCompanyTrendChartBody:
    ...
    if "pgCompanies" not in obj:
        raise KeyError(f"'pgCompanies' is not in {obj}")
    ...
    for i, item in enumerate(pg_companies):
        ...
        pg_companies[i] = item
    return GetAnalyticsPgCompanyTrendChartBody(from_, until, currency, exclude_cancelled, time_granularity, pg_companies)
