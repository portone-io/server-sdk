...
@dataclass
class PlatformPartnerOrderSettlement:
    id: str
    """정산내역 아이디
    """
    graphql_id: str
...
def _serialize_platform_partner_order_settlement(obj: PlatformPartnerOrderSettlement) -> Any:
...
def _deserialize_platform_partner_order_settlement(obj: Any) -> PlatformPartnerOrderSettlement:
    ...
    type = obj["type"]
    if type != "ORDER":
        raise ValueError(f"{repr(type)} is not 'ORDER'")
    if "id" not in obj:
    ...
    if "memo" in obj:
        ...
        memo = None
    return PlatformPartnerOrderSettlement(id, graphql_id, partner, settlement_date, settlement_currency, status, contract, settlement_start_date_range, amount, is_for_test, status_updated_at, memo)
