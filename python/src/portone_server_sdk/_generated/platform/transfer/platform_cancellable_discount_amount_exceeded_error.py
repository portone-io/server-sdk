...
@dataclass
class PlatformCancellableDiscountAmountExceededError:
    type: Literal["PLATFORM_CANCELLABLE_DISCOUNT_AMOUNT_EXCEEDED"] = field(repr=False)
...
def _serialize_platform_cancellable_discount_amount_exceeded_error(obj: PlatformCancellableDiscountAmountExceededError) -> Any:
    entity = {}
...
def _deserialize_platform_cancellable_discount_amount_exceeded_error(obj: Any) -> PlatformCancellableDiscountAmountExceededError:
    if not isinstance(obj, dict):
        raise ValueError(f"{repr(obj)} is not dict")
    if "type" not in obj:
        raise KeyError(f"'type' is not in {obj}")
    type = obj["type"]
    if type != "PLATFORM_CANCELLABLE_DISCOUNT_AMOUNT_EXCEEDED":
        raise ValueError(f"{repr(type)} is not 'PLATFORM_CANCELLABLE_DISCOUNT_AMOUNT_EXCEEDED'")
    ...
    if "message" in obj:
        ...
        message = None
    return PlatformCancellableDiscountAmountExceededError(type, discount_share_policy_id, discount_share_policy_graphql_id, cancellable_amount, request_amount, product_id, message)
